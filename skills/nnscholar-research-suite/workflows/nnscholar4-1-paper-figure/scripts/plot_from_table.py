...
def load_pandas():
    try:
        ...
        raise SystemExit("Missing dependency: pandas. Install pandas to read structured tables.") from exc
    return pd


def load_plot_dependencies():
    missing_message = (
        "Missing plotting dependency. Install matplotlib, numpy, pandas, and seaborn "
        "to generate figures with plot_from_table.py."
    )
    try:
        ...
        raise SystemExit(f"{missing_message} First missing module: {exc.name}") from exc
...
def make_plot(df: pd.DataFrame, args: argparse.Namespace) -> plt.Figure:
    plt, np, pd, sns = load_plot_dependencies()
